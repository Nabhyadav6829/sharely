...
//   const [status, setStatus] = useState("Enter a room code to start.");
//   const [members, setMembers] = useState(0);
//   const [sharedFiles, setSharedFiles] = useState([]);
//   const [deviceName, setDeviceName] = useState(defaultName());

//   const socketRef = useRef(null);
//   const pcRefs = useRef(new Map()); // Map to store peer connections by socket ID
//   const dcRefs = useRef(new Map()); // Map to store data channels by socket ID
//   const fileReceiverRef = useRef(new Map()); // Map to store file reception state by peer ID
...
//       setSharedFiles((prev) => {
//         // Only add if not already present from this sender
//         if (!prev.some(f => f.name === fileInfo.name && f.senderId === fileInfo.senderId)) {
//           return [...prev, { ...fileInfo, url: null }]; // Initialize with no URL
//         }
//         return prev;
//       });
//     });
...
//       fileReceiverRef.current.delete(peerId); // Clean up file receiver state
//     });
...
//     channel.onopen = () => setStatus(`Data channel open with ${peerId}. Ready to send files.`);
//     channel.onclose = () => {
...
//       senderId: socketRef.current.id // Include sender's socket ID
//     };
...
//           // Update sharedFiles with the URL for this file
//           setSharedFiles((prev) => {
...
//                   url, 
//                   sender: receiver.metadata.senderId || 'Unknown',
...
//           setProgress(0);
//           fileReceiverRef.current.delete(peerId); // Clean up
//         }
//       }
//     } catch (error) {
...
//         <h1 className="text-4xl font-bold text-center text-teal-400">LAN Share 🚀</h1>
...
//             className="bg-gray-700 text-white placeholder-gray-400 border-2 border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-teal-500 transition w-full mr-2"
//           />
...
//               className="bg-gray-700 text-white placeholder-gray-400 border-2 border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-teal-500 transition"
//             />
//             <button
//               onClick={joinRoom}
//               className="bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-4 rounded-lg transition duration-300 ease-in-out"
//             >
//               Join Room
//             </button>
//           </div>
...
//               Room: <span className="font-bold text-teal-400">{room}</span>
//             </p>
//             <p className="text-center text-sm text-gray-300 bg-gray-700 rounded-md p-2">{status}</p>
//             <div>
//               <label htmlFor="file-upload" className="block text-sm font-medium text-gray-300 mb-2">Select a file to send:</label>
//               <input
//                 id="file-upload"
//                 type="file"
//                 onChange={(e) => setFile(e.target.files[0])}
//                 className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-100 file:text-teal-700 hover:file:bg-teal-200 cursor-pointer"
//               />
//             </div>
//             <button
//               onClick={sendFile}
//               disabled={!file || dcRefs.current.size === 0}
//               className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg transition duration-300 disabled:bg-gray-600 disabled:cursor-not-allowed"
//             >
//               Send File
//             </button>
//             {progress > 0 && (
...
//               <div className="mt-4">
//                 <h2 className="text-lg font-semibold text-gray-300 mb-2">Shared Files</h2>
//                 <ul className="space-y-2 max-h-60 overflow-y-auto">
//                   {sharedFiles.map((f, i) => (
//                     <li key={`${f.name}-${f.senderId}-${i}`} className="flex items-center justify-between bg-gray-700 rounded-lg p-3">
//                       <div>
//                         <div className="font-medium text-teal-300">{f.name}</div>
//                         <div className="text-xs text-gray-400">Size: {prettyBytes(f.size)} • From: {f.sender}</div>
//                       </div>
//                       {f.url && (
//                         <a href={f.url} download={f.name} className="px-3 py-1 rounded bg-teal-600 text-white hover:bg-teal-700">
//                           Download
//                         </a>
//                       )}
//                     </li>
